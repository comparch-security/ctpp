...
ps_evset_reduce(Elem **evset, char *victim, int len, int threshold){
  ...
  for (i=0; i<list_len; i++) {
    ...
    if (ps_evset_test(evset, victim, threshold, 50, EVTEST_MEDIAN)) {
...
/*
int ctpp_ps_evset  (Elem **evset, char *victim, int len, uint64_t* page, int is_huge, int threshold, int* evset_len){

  
  int len_requested = len;
  int i;
  int try_guesses, time;
  int prime_len = 0;
  int p1miss_list_len = 0;

  int guess_index = 0;
  Elem *prime_list;
  Elem **prime_list_ptr = &prime_list;
  *prime_list_ptr = NULL;
  Elem *evset_last = NULL;
  *evset = NULL;
  //////////////////////////////////////////////////////////////////////////////
...
extend:
  while (*evset_len < len) {
    ...
    while(try_guesses){
      ...
      // Measure TARGET
      time = time_mread((void*)victim);
      prime_len = list_append(prime_list_ptr,(Elem*)guess_pool[guess_index]);  
      guess_index++; 
      // If TARGET is evicted
      #if IGNORE_VERY_SLOW == 1
      if (time > threshold-20 && time < 2*threshold) { 
      ...
      if (time > threshold-20) { 
      #endif
        try_guesses = false; // unhit
        // asm volatile("lfence;mfence");
        // maccess((void*)victim);
        // asm volatile("lfence;mfence");
      }
    }
    uint64_t *prime_array = (uint64_t*)malloc(sizeof(uint64_t)*prime_len);
    list_to_array(*prime_list_ptr,prime_array);
    Elem* p1miss_list;
    Elem** p1miss_list_ptr = &p1miss_list;
    *p1miss_list_ptr = NULL;
    
    // CTPP STEP2;
    for(int i = 0 ; i < prime_len ; i++){
      asm volatile("lfence;mfence");
      time = time_mread((void*)prime_array[i]);
      asm volatile("lfence;mfence");
      // maccess((void*)prime_array[i]);
      // asm volatile("lfence;mfence");

      #if IGNORE_VERY_SLOW == 1
      if ((time>threshold-20 && time < 2*threshold)) { 
      #else
      if ((time>threshold-20)) { //unhit
      #endif
        Elem* p1miss_list_last = (Elem*)prime_array[i];
        p1miss_list_len = list_append(p1miss_list_ptr,p1miss_list_last);
      }

    }
    uint64_t *p1miss_array = (uint64_t*)malloc(sizeof(uint64_t)*p1miss_list_len);
    list_to_array(*p1miss_list_ptr,p1miss_array);
    free(prime_array);

    // CTPP STEP3: 
    for(int i = 0 ; i < p1miss_list_len ; i++){
      asm volatile("lfence;mfence");
      time = time_mread((void*)p1miss_array[i]);
      asm volatile("lfence;mfence");
      // maccess((void*)p1miss_array[i]);
      // asm volatile("lfence;mfence");

      #if IGNORE_VERY_SLOW == 1
      if (!(time>threshold-20 && time < 2*threshold)) { 
      #else
      if (!(time>threshold-20)) { //hit
      #endif
        Elem* evset_last = (Elem*)p1miss_array[i];
        *evset_len = list_append(evset,evset_last);
      }
    }
    // printf("evset_len:%d\n",*evset_len);
    free(p1miss_array);
  }
  printf("\tprime_len:%d p1miss_list_len:%d,evset_len:%d,%p %p\n",prime_len,p1miss_list_len,*evset_len,*evset,(*evset)->next);
  if(ps_evset_test(evset,victim,threshold,50,EVTEST_MEDIAN)){
    return 1;
  }
  else{
